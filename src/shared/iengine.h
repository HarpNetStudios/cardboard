...
extern Uint64 curtime;          // scaled duration since last step
extern Uint64 lastmillis;       // totalmillis value at last step
extern Uint64 elapsedtime;      // wall clock duration since last step
...
#define liquididx(mat) (isliquid(mat) ? ((mat % 4) + 1) : 0)
...
extern int lookuptexture(const vec& o, int orient);
...
extern void particle_textcopy(const vec &s, const char *t, int type, int fade = 2000, int color = 0xFFFFFF, float size = 2.0f, int gravity = 0, int distance = 0);
...
extern void renderclient(dynent *d, const char *mdlname, modelattach *attachments, int hold, int attack, int attackdelay, Uint64 lastaction, Uint64 lastpain, float fade = 1, bool ragdoll = false);
...
enum { DISC_NONE = 0, DISC_EOP, DISC_LOCAL, DISC_KICK, DISC_MSGERR, DISC_IPBAN, DISC_PRIVATE, DISC_MAXCLIENTS, DISC_TIMEOUT, DISC_OVERFLOW, DISC_PASSWORD, DISC_PUBTOKEN, DISC_NUM };
...
namespace steam
{
	...
	extern void addTimelineEvent(const char* title, const char* desc, const char* icon = "");
	extern void setTimelineMode(int mode);
