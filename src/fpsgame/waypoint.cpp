...
namespace ai
{
    ...
    static inline void invalidatewpcache(int wp)
    {
        ...
        {
            for(int i = 0; i < int(WPCACHE_DYNAMIC); ++i) if(wp >= wpcaches[i].firstwp && wp <= wpcaches[i].lastwp) { invalidatedwpcaches |= 1<<i; return; }
    ...
    int closestwaypoint(const vec &pos, float mindist, bool links, fpsent *d)
    {
        ...
        for(int which = 0; which < NUMWPCACHES; ++which) if(wpcaches[which].firstwp >= 0) for(curnode = &wpcaches[which].nodes[0], wpcachestack.setsize(0);;)
        {
    ...
    bool getwaypointfile(const char *mname, char *wptname)
    {
        ...
        oldstring pakname, mapname, cfgname;
