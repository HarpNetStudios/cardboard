...
{
    for(int i = 0; i < 7; ++i) if(children[i].flags!=children[7].flags) return false;
    bvec bbs[4];
...
static void genpvsnodes(cube *c, int parent = 0, const ivec &co = ivec(0, 0, 0), int size = worldsize/2)
{
    ...
    int branches = 0;
    for(int i = 0; i < 8; ++i) if(c[i].children)
    {
        ivec o(i, co, size);
...
struct shaft
{
    ...
    void calcshaft(const shaftbb &from, const shaftbb &to)
    {
        ...
        numplanes = 0;
        for(int i = 0; i < 5; ++i) if(!(match&(1<<i))) for(int j = i+1; j<6; j++) if(!(match&(1<<j)) && i+3!=j && ((color>>i)^(color>>j))&1)
        {
...
struct pvsworker
{
    ...
    void hidepvs(pvsnode &p)
    {
        if(p.children)
        {
            ...
            for(int i = 0; i < 8; ++i) hidepvs(children[i]);
            p.flags |= PVS_HIDE_BB;
            return;
        }
        p.flags |= PVS_HIDE_BB;
    ...
    bool compresspvs(pvsnode &p, int size, int threshold)
    {
        ...
        if(canreduce)
        {
            ...
            for(int i = 0; i < 7; ++i) if((children[i].flags&PVS_HIDE_BB)!=hide) canreduce = false;
            if(canreduce)
            {
    ...
    bool serializepvs(pvsnode &p, int storage = -1)
    {
        ...
        {
            outbuf.add(0xFF);
            for(int i = 0; i < 8; ++i) outbuf.add(p.flags&PVS_HIDE_BB ? 0xFF : 0);
            return true;
        }
        ...
        outbuf.add(0);
        for(int j = 0; j < 8; ++j) outbuf.add(leafvalues&(1<<j) ? 0xFF : 0);
    ...
    void calcpvs(const ivec &co, int size)
    {
        ...
        waterbytes = 0;
        for(int i = 0; i < 4; ++i) if(wateroccluded&(0xFF<<(i*8))) waterbytes = i+1;
...
struct viewcellnode
{
    ...
    {
        for(int i = 0; i < 8; ++i) children[i].pvs = -1;
    }
    ~viewcellnode()
    {
        for(int i = 0; i < 8; ++i) if(!(leafmask&(1<<i))) delete children[i].node;
    }
};
...
{
    for(int k = 0; k < 3; ++k) pvsbounds.min[k] = USHRT_MAX;
    for(int k = 0; k < 3; ++k) pvsbounds.max[k] = 0;
    loopv(valist)
    {
