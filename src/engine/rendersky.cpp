...
namespace fogdome
{
    ...
    void subdivide(int depth, int face)
    {
        if(depth-- <= 0) return;
        int idx[6];
        for(int i = 0; i < 3; ++i) idx[i] = indices[face+2-i];
        for(int i = 0; i < 3; ++i)
        {
        ...
        subdivide(depth, face);
        for(int i = 0; i < 3; ++i) genface(depth, idx[i], idx[3+i], idx[3+(i+2)%3]);
    }
    ...
    void init(const bvec &color, float minalpha = 0.0f, float maxalpha = 1.0f, float capsize = -1, float clipz = 1, int hres = 16, int depth = 2)
    {
        ...
        if(clipz >= 1)
        {
            ...
            for(int i = 0; i < hres; ++i) verts[numverts++] = vert(vec(sincos360[(360*i)/hres], 0.0f), color, maxalpha);
            for(int i = 0; i < hres; ++i) genface(depth, 0, i+1, 1+(i+1)%hres);
        }
        else if(clipz <= 0)
        {
...
static void drawatmosphere(int w, float z1clip = 0.0f, float z2clip = 1.0f, int faces = 0x3F)
{
    ...
    if(glaring)
    {
        if(sundiskscale > 0 && sunlightdir.z*w + sundiskscale > z1 && sunlightdir.z*w - sundiskscale < z2)
        {
            ...
            for(int i = 0; i < 4; ++i) gle::attrib(vec(spoke).rotate(-2*M_PI*i/4.0f, sunlightdir).add(sunlightdir).mul(w));
            xtraverts += gle::end();
        }
        return;
    }
