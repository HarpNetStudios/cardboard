...
struct skelmodel : animmodel
{
    ...
    struct blendcombo
    {
        ...
        {
            for(int k = 0; k < 4; ++k) if(bones[k] != c.bones[k]) return false;
            for(int k = 0; k < 4; ++k) if(weights[k] != c.weights[k]) return false;
            return true;
        }
        ...
        void serialize(vvertw &v)
        {
            ...
            {
                v.weights[0] = 255;
                for(int k = 0; k < 3; ++k) v.weights[k+1] = 0;
                v.bones[0] = 2*interpindex;
                for(int k = 0; k < 3; ++k) v.bones[k+1] = v.bones[0];
            }
            else
            {
                int total = 0;
                for(int k = 0; k < 4; ++k) total += (v.weights[k] = uchar(0.5f + weights[k]*255));
                while(total > 255)
                {
                    for(int k = 0; k < 4; ++k) if(v.weights[k] > 0 && total > 255) { v.weights[k]--; total--; } 
                }
                while(total < 255)
                {
                    for(int k = 0; k < 4; ++k) if(v.weights[k] < 255 && total < 255) { v.weights[k]++; total++; }
                }
                for(int k = 0; k < 4; ++k) v.bones[k] = 2*interpbones[k];
            }
        }
    };
    ...
    struct skelmesh : mesh
    {
        ...
        int genvbo(vector<ushort> &idxs, int offset, vector<T> &vverts)
        {
            ...
            for(int i = 0; i < int(numtris); ++i) for(int j = 0; j < 3; ++j) idxs.add(voffset + tris[i].vert[j]);
        ...
        int genvbo(vector<ushort> &idxs, int offset)
        {
            ...
            {
                tri &t = tris[i];
                for(int j = 0; j < 3; ++j) idxs.add(voffset+t.vert[j]);
            }
            minvert = voffset;
            maxvert = voffset + numverts-1;
            elen = idxs.length()-eoffset;
    ...
    struct skeleton
    {
        ...
        void genragdollbones(ragdolldata &d, skelcacheentry &sc, part *p)
        {
            ...
            loopv(ragdoll->joints)
            {
                ...
                const boneinfo &b = bones[j.bone];
                vec pos(0, 0, 0);
                for(int k = 0; k < 3; ++k) if(j.vert[k]>=0) pos.add(d.verts[j.vert[k]].pos);
