...
struct gui : g3d_gui
{
	...
	bool is_hit(int w, int h, int x = curx, int y = cury)
	{
	...
	int image(Texture *t, float scale, const char *overlaid)
	{
		...
		if(visible()) icon_(t, overlaid!=NULL, curx, cury, size, is_hit(size+SHADOW, size+SHADOW), overlaid);
	...
	int texture(VSlot &vslot, float scale, bool overlaid)
	{
		...
		if(scale==0) scale = 1;
		int size = (int)(scale*2*FONTH)-SHADOW;
		if(visible()) previewslot(vslot, overlaid, curx, cury, size, is_hit(size+SHADOW, size+SHADOW));
		return layout(size+SHADOW, size+SHADOW);
	}
	...
	int playerpreview(int model, int team, int weap, float sizescale, const char *overlaid)
	{
		...
		if(model>=0 && visible())
		{
			bool hit = is_hit(size+SHADOW, size+SHADOW);
