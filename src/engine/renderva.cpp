...
{
    for(int i = 0; i < 4; ++i) if(vfcP[i].dist(cv) < -rad) return true;
...
{
    for(int i = 0; i < 5; ++i) if(o.dist(vfcP[i]) < -vfcDfar[i]*size) return true;
    return false;
}

static inline int isfoggedcube(const ivec &o, int size)
{
    for(int i = 0; i < 4; ++i) if(o.dist(vfcP[i]) < -vfcDfar[i]*size) return true;
...
void calcvfcD()
{
    for(int i = 0; i < 5; ++i)
    {
        ...
        for(int k = 0; k < 3; ++k) if(p[k] > 0) vfcDfar[i] += p[k];
        else vfcDnear[i] += p[k];
    }
} 
...
void setvfcP(float z, const vec &bbmin, const vec &bbmax)
{
    ...
    if(z >= 0) for(int i = 0; i < 5; ++i) vfcP[i].reflectz(z);
...
static void setupbb()
{
    if(!bbvbo)
    {
        ...
        gle::bindvbo(bbvbo);
        vec verts[8];
        for(int i = 0; i < 8; ++i) verts[i] = vec(i&1, (i>>1)&1, (i>>2)&1);
...
void renderdepthobstacles(const vec &bbmin, const vec &bbmax, float scale, float *ranges, int numranges)
{
    ...
    else
    {
        ...
        if(!numranges) for(int i = 0; i < 4; ++i) scales[i] = 1.0f/scale;
...
struct renderstate
{
    ...
    {
        for (int k = 0; k < 8; ++k) textures[k] = 0;
    }
};
...
void renderalphageom(bool fogpass)
{
    ...
    for(int front = 0; front < 2; ++front) if(front || hasback)
    {
