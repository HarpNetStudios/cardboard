...
hashnameset<userauthinfo> users;

void adduser(char *name, char *pubkey)
{
    name = newstring(name);
    userauthinfo&u = users[name];
    u.name = name;
...
void clearusers()
{
    enumerate(users, userauthinfo, u, { delete[] u.name; freepubkey(u.pubkey); });
    users.clear();
}
COMMAND(clearusers, "");
...
void setupserver(int port, const char *ip = NULL)
{
    ...
    masterstarttime = time(NULL);
    char *ct = ctime(&masterstarttime);
    if(strchr(ct, '\n')) *strchr(ct, '\n') = '\0';
...
void reqauth(client &c, uint id, char *name)
{
    ...
    userauthinfo *u = users.access(name);
    if(!u)
    {
        outputf(c, "failauth %u\n", id);
        return;
    ...
    a.reqtime = servtime;
    a.id = id;
    uint seed[3] = { uint(masterstarttime), servtime, randomMT() };
